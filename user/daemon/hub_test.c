...
static void dump_snapshot(const struct sh_snapshot *snap)
{
    ...
    for (i = 0; i < snap->count; ++i) {
        ...
        printf("  sensor id=%u type=%u flags=0x%x nvalues=%u ts=%lld",
               v->id, v->type, v->flags, v->nvalues,
...
int main(void)
{
    ...
    if (sensor_client_get_info(&cli, &info) == 0) {
        printf("hub info: version=0x%x sensors=%u qsize=%u qdepth=%u event_seq=%llu snapshot_seq=%llu\n",
               info.version,
               info.sensor_count,
               info.queue_size,
               info.queue_depth,
